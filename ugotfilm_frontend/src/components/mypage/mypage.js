    ...
    <div className="user_contents_wrap">
      ...
      <div className="user_contents user_tag">
        ...
        <div className="best_one">
          <span>{localStorage.getItem('nickname')}</span>님이 선호하는
          태그입니다.
          {/* <Wordcloud /> */}
          {wordcloud === null ? null : 
          <div style={{ height: '250px', width: '100%' }}>
          <ReactWordcloud callbacks={callbacks} words={wordcloud} options={options} size={size}/>
        </div>
          }
        ...
        <div className="user_actor_list" >
          {cast && cast.map((element, idx) => (
            <div className="list_intro" key={idx}>
                ...
                >
              <span>
                {element.profile_url == null || element.profile_url == undefined ? (
      ...
      <div className="user_contents user_director">
        ...
        <div className="user_actor_list" >
          {director && director.map((element, idx) => (
            <div className="list_intro" key={idx}>
              <NavLink
                to={`/person/${element.personcode}`}
                style={{ textDecoration: 'none' }}
      ...
      <div className="user_contents user_genre">
        <div className="user_genre_list">
          ...
          <div>
            {genre && genre.map((element, idx) => (
              <div className="list_top3" key={idx}>    
              <NavLink to={`/genre/pop/${element.personcode}`} style={{ textDecoration: 'none' }}>
                <span className="list_top3_name">{element.name}</span>
                <span className="list_top3_count">{element.count}</span>
                </NavLink>
              </div>
            ))}
          </div>
